 ...
 * verify-api-keys.ts
 * 
 * Lightweight smoke test for all 3rd-party API keys.
 * Each check does the cheapest possible call to verify the credential is valid.
 * 
 * Usage:  npx ts-node scripts/verify-api-keys.ts
 */
...
// ─── 7. Google OAuth (client ID format check) ───────────
