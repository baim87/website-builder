...
3. It must accept a single \`data\` prop which contains all the text, copy, and image content. 
   - CRITICAL SCHEMA RULE: Assume \`data\` EXACTLY matches the JSON Schema contract above. DO NOT invent props like \`data.headline\` or \`data.items\` unless they exist in the schema.
   - CRITICAL BLOCK IDs: The \`data\` object contains a shadow \`_blockIds\` object that mirrors the data structure (e.g. if \`data.headline\` exists, \`data._blockIds.headline\` exists and contains a UUID). You MUST attach \`data-block-id={data._blockIds?.propertyName}\` AND \`data-block-name="propertyName"\` to the specific DOM element (like \`<h1>\`, \`<p>\`, \`<img>\`, \`<button>\`) that renders that property. If mapping over an array like \`data.services\`, use \`data._blockIds?.services?.[index]?.propertyName\` and \`data-block-name="services"\`. Always use optional chaining when accessing \`_blockIds\`.
...
12. IF generating a HeaderSection, you MUST render a compact, top-fixed navigation bar containing a logo (or business name), a row of navigation links, and a primary CTA button. Do NOT render a large hero banner or massive headline. It MUST be a navigation bar (e.g. \`<nav>\`). CRITICAL: If \`data.phone\` is provided, you MUST render it visibly inline as a prominent text link (e.g. \`<a href={"tel:" + data.phone}>\`) within the desktop navigation bar alongside the CTA button. Do NOT hide the phone number in a dropdown. CRITICAL BACKGROUND COLOR: You MUST use \`bg-header-bg text-header-foreground\` for the header background and text colors instead of generic \`bg-background\`. You MUST apply \`text-header-foreground\` to the root header element AND to all navigation links (do NOT use \`text-foreground\`).
13. IF generating a FooterSection, you MUST render a standard footer containing contact info, quick links, and a copyright notice. CRITICAL BACKGROUND COLOR: You MUST use \`bg-footer-bg text-footer-foreground\` for the footer wrapper instead of generic \`bg-background\` or \`bg-surface-dark\`. You MUST apply \`text-footer-foreground\` to the root footer element AND to all text and links (do NOT use \`text-surface-dark-foreground\` or \`text-foreground\`). Do NOT render a large hero banner or massive headline. It MUST be a footer (e.g. \`<footer>\`). CRITICAL LAYOUT RULE: You MUST render the main footer content as a strict 4-column grid on desktop (e.g., \`grid md:grid-cols-4 gap-8\`). The 4 columns MUST be: 1) Brand Info & Social Links, 2) Contact Info (Phone, Email, Address), 3) "Navigate" (quickLinks), and 4) "Services" (serviceLinks). Do NOT stack the contact info under the brand info on desktop; they must be distinct columns. If 'developerCredit' and 'developerLink' exist, you MUST render them as a subtle link (e.g., \`<a href={data.developerLink} target="_blank" rel="noopener noreferrer">\`) next to or below the copyright text.
...
15. IF generating a LeadFormSection, you MUST render an actual HTML \`<form>\` based on the \`data.fields\` array. Include styled \`<input>\` and \`<textarea>\` elements, and a prominent submit button using \`data.submitText\`. Ensure the form looks professional and trustworthy. IMPORTANT: You MUST implement a \`submitted\` state. When \`submitted\` is true, hide the form and display a clear, well-styled success message with a checkmark icon and text. CRITICAL ERROR HANDLING: If the fetch response is not ok (e.g. \`!res.ok\`), you MUST extract the error message from the response JSON (\`data.error\`) and display it in a prominent red error banner above the form, or use \`window.alert(data.error)\`.
    - SERVICES FIELD RULE: You MUST render the \`services\` field (if present in \`data\`) as a multi-select dropdown with search functionality. Use the \`react-select\` library (\`import Select from 'react-select'\`). Render this dropdown immediately next to the "Postal Code" or "Zip" field in a 2-column grid. Make sure the styles match the other Tailwind inputs.
    - TEXTAREA RULE: You MUST ensure the textarea for the project description has the \`name="message"\` attribute (do NOT use "projectdescription").
    - HONEYPOT RULE: You MUST include a visually hidden input field to catch spam bots. Generate \`<input type="text" name="website_url" style={{ display: 'none' }} tabIndex={-1} autoComplete="off" />\` inside the form.
    - UTM TRACKING RULE: In your form submission handler, BEFORE calling \`fetch('/api/lead')\`, check \`sessionStorage.getItem('utm_source')\` and \`sessionStorage.getItem('utm_campaign')\`. If they exist, append them to the \`FormData\` as \`utm_source\` and \`utm_campaign\` respectively.
...
17. IF generating a BrandsSection, you MUST render the logos cleanly. Use CSS filters (e.g., 'grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all') to make the logos look uniform and professional. Do NOT use grid borders, divide lines (\`divide-x\`, \`divide-y\`), or card borders around the logos. Keep it as a clean, open layout with plenty of whitespace.
